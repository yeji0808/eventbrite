...
module.exports = function(passport) {
  ...
  passport.use('local-signin', new LocalStrategy({
    ...
    try {
      ...
      if (user && await user.validatePassword(password)) {
        return done(null, user, req.flash('success', '성공적으로 로그인 했습니다.'));
      }
      return done(null, false, req.flash('danger', '아이디나 비밀번호를 확인해 주세요.'));
    } catch(err) {
      done(err);
    }
  }));
  ...
  passport.use(new FacebookStrategy({
    ...
    clientID : '497633000622968',
    clientSecret : '11faebb75f195370b506e1ba8cb3c100',
    callbackURL : 'https://glacial-forest-47226.herokuapp.com/auth/facebook/callback',
    ...
    try {
      ...
      var name = (profile.displayName) ? profile.displayName : 
        [profile.name.givenName, profile.name.middleName, profile.name.familyName]
          .filter(e => e).join(' ');
      console.log('페이스북 정보들',email, picture, name, profile.name);
